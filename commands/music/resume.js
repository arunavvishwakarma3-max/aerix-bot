...
export default {
  ...
  async execute(interaction, client) {
    ...
    if (!queue.paused) {
      return interaction.reply({ embeds: [createEmbed({ color: config.colors.warning, description: `${config.emoji.music || '🎵'} Already playing.` })], ephemeral: true });
    }
    ...
    await interaction.reply({ embeds: [createEmbed({ color: config.colors.success, description: `${config.emoji.music || '🎵'} Resumed.` })] });
  },
};
